...
class GrillberDataLoader {
  ...
  getAvailableProduct(date) {
    return this.query(
      ...
      .where(`${date} < b.dropDate or ${date} > b.pickUpDate`)
      .toString()
      // .select(
      //   'p.id',
      //   'p.category',
      ...
      //   )
      // .from('products AS p')
      // .join('booked_products AS b_p')
      // .on('p.id = b_p.productId')
      // .join('bookings AS b')
      // .on('b.id = b_p.bookingId')
      // .where(`p.available = 1 and ${date} < b.dropDate or ${date} > b.pickUpDate`)
      // .toString()
    );
  }
